...
print("cspline.py:", os.path.join(os.path.dirname(__file__), 'cbspline'))
# MAC
_cbspline = np.ctypeslib.load_library(os.path.join(os.path.dirname(__file__), 'cbspline.so'), '.')
# Linux
#_cbspline = np.ctypeslib.load_library(os.path.join(os.path.dirname(__file__), 'cbspline'), '.')

#_cbspline.polyfit.restype = ctypes.c_int
_cbspline.cbsplcoeff.argtypes = [ ptr(dtype=float), # x
                                 ptr(dtype=float),  # G
                                 ptr(dtype=int),    # kk
                                 c_int]    #n
_cbspline.lhsbnd_fill.argtypes = [ptr(dtype=float), # lhsbnd
                                 ptr(dtype=float),  # G
                                 ptr(dtype=float),  # w
                                 ptr(dtype=int),    # kk
                                 c_int,    # n
                                 ...
                                 c_int,    # nk
                                 c_double] #lam
_cbspline.rhs_fill.argtypes = [ptr(dtype=float),    # rhs
                                 ptr(dtype=float),  # G
                                 ptr(dtype=float),  # w
                                 ptr(dtype=int),    # kk
                                 c_int]                # n
_cbspline.bandsol.argtypes = [ptr(dtype=float),     # rhs
                                 ptr(dtype=float),  # G
