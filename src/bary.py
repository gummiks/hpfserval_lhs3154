...
def bjdbrv(jd_utc, ra=None, dec=None, obsname=None, lat=0., lon=0., elevation=None,
   ...
   if obsname=="McDonald Observatory": # Same as used in SERVAL
       ...
       #lon = -104.0147
       #elevation = 2025.
       lat = serval_config.instruments["HPF"]["latitude"]
       lon = serval_config.instruments["HPF"]["longitude"]
       elevation = serval_config.instruments["HPF"]["altitude"]
   ...
   brv, warning, status = barycorrpy.get_BC_vel(JDUTC, ra=ra, dec=dec, epoch=epoch, pmra=pmra,
                   pmdec=pmdec, px=parallax, lat=lat, longi=lon, alt=elevation, leap_update=False,**kwargs)
   print('########################################')
   print('NOTE: Setting ****LEAP_UPDATE=FALSE****')
   print('########################################')
