...
import cubicSpline
from serval_config import def_wlog
from serval_config import spline_cv, spline_ev
...
def calcspec(x2, v, *a, **kwargs):
   ...
   p(x,a) * f(x, v)
   Calculates an interpolated spectrum using x2, v, and calcspec.tck
   - i have no idea why calcspec.tck is not passed as an argument here -- it contains the derivatives needed to do the interpolation. Is this what is being unstable, the interpolation ?
   ...
   OUTPUT:
    fmod*poly - the flux*the polynomial
   """
   ...
   if not kwargs.get('retpoly'):
      #print("calcspec.cubicSpline.spl_evf")
      # Calculate interpolated spline 
      #fmod = kwargs.get('fmod', cubicSpline.spl_evf(dopshift(x2,v),calcspec.tck)) # parsed fmod or new
      #GKS
      fmod = kwargs.get('fmod', spline_ev(dopshift(x2,v),calcspec.tck)) # parsed fmod or new
   x2 = x2 - calcspec.wcen
